# 05 — QUESTIONS OUVERTES ET DÉCISIONS ACTÉES

## Décisions actées (validées par le porteur de projet le 07/07/2026)
1. **Attributs** — les 5 attributs du GDD sont validés (Puissance, Concentration, Résistance, Vitalité, Sagesse).
2. **Écoles de magie** — écoles conservées, version modernisée, choisies à la création du personnage.
3. **Combat** — le joueur définit une liste de sorts ; le combat se résout automatiquement après un **minuteur de 10 minutes** ; le rapport montre ensuite l'ordre des sorts joués. Algorithme de choix le plus simple possible.
4. **Actions** — quota d'actions **par jour** (pas de jauge d'énergie qui se recharge).
5. **Banque** — confirmée : l'or déposé est protégé du vol PvP.
6. **Quêtes à minuteur** — confirmées, le personnage est occupé pendant la mission.
7. **Alchimie** — sert au **reroll** des objets et des sorts (pas de fabrication de potions).
8. **Gemmes** — monnaie premium **100 % cosmétique** (skins, photos de profil, bannières), modèle Riot Games. Aucun avantage de gameplay.
9. **Confort moderne** — accepté (journalières, notifications, donjons persistants) tant que le cœur reste fidèle.
10. **Vitesse de jeu** — rythme fidèle 2008 (lent), profil `pacing: classic` en configuration.
11. **PvP** — modèle original conservé : attaques non consenties avec protections.
